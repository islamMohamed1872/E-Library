    ...
    return BlocConsumer<EbookCubit,EbookStates>(
        builder: (context,state){
          ...
          return Scaffold(
            ...
            child:  cubit.horrorSnapshot==null?FutureBuilder(
                future: EbookCubit.get(context).getCategoryBooks(category: "Horror"),
                builder: (BuildContext context,AsyncSnapshot snapshot) {
                  if (snapshot.hasData) {
                    cubit.horrorSnapshot = snapshot;
              ...
              childAspectRatio: 1/1.7,
              children:
              List.generate(cubit.horrorSnapshot.data['data'].length, (index) => buildGridBook(
                onTap: (){
                  EbookCubit.get(context).index = index;
                  EbookCubit.get(context).isSaved = false;
                  EbookCubit.get(context).books = cubit.horrorSnapshot.data['data'][index];
                  EbookCubit.get(context).bookModel = BookModel.fromJson(
                      cubit.horrorSnapshot.data['data'][index]);
                  navigateTo(context, PreviewBookScreen());
                },
                bookModel: BookModel.fromJson(
                    cubit.horrorSnapshot.data['data'][index]),
                context: context,
                snapshot: cubit.horrorSnapshot,
              ),
              ),
            ),
            ),
