    ...
    return BlocConsumer<EbookCubit,EbookStates>(
        builder: (context,state){
          ...
          return Scaffold(
            ...
            child:  cubit.romanceSnapshot==null?FutureBuilder(
                future: EbookCubit.get(context).getCategoryBooks(category: "Romance"),
                builder: (BuildContext context,AsyncSnapshot snapshot) {
                  if (snapshot.hasData) {
                    cubit.romanceSnapshot = snapshot;
              ...
              childAspectRatio: 1/1.7,
              children:
              List.generate(cubit.romanceSnapshot.data['data'].length, (index) => buildGridBook(
                onTap: (){
                  EbookCubit.get(context).index = index;
                  EbookCubit.get(context).isSaved = false;
                  EbookCubit.get(context).books = cubit.romanceSnapshot.data['data'][index];
                  EbookCubit.get(context).bookModel = BookModel.fromJson(
                      cubit.romanceSnapshot.data['data'][index]);
                  navigateTo(context, PreviewBookScreen());
                },
                bookModel: BookModel.fromJson(
                    cubit.romanceSnapshot.data['data'][index]),
                context: context,
                snapshot: cubit.romanceSnapshot,
              ),
              ),
            ),
            ),
