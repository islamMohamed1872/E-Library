    ...
    return BlocConsumer<EbookCubit,EbookStates>(
        builder: (context,state){
          ...
          return Scaffold(
            ...
            child:  cubit.fantasySnapshot==null?FutureBuilder(
                future: EbookCubit.get(context).getCategoryBooks(category: "Fantasy"),
                builder: (BuildContext context,AsyncSnapshot snapshot) {
                  if (snapshot.hasData) {
                    cubit.fantasySnapshot = snapshot;
              ...
              childAspectRatio: 1/1.7,
              children:
              List.generate(cubit.fantasySnapshot.data['data'].length, (index) => buildGridBook(
                onTap: (){
                  EbookCubit.get(context).index = index;
                  EbookCubit.get(context).isSaved = false;
                  EbookCubit.get(context).books = cubit.fantasySnapshot.data['data'][index];
                  EbookCubit.get(context).bookModel = BookModel.fromJson(
                      cubit.fantasySnapshot.data['data'][index]);
                  navigateTo(context, PreviewBookScreen());
                },
                bookModel: BookModel.fromJson(
                    cubit.fantasySnapshot.data['data'][index]),
                context: context,
                snapshot: cubit.fantasySnapshot,
              ),
              ),
            ),
            ),
