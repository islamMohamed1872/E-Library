    ...
    return BlocConsumer<EbookCubit,EbookStates>(
        builder: (context,state){
          ...
          return Scaffold(
            ...
            child:  cubit.scienceSnapshot==null?FutureBuilder(
                future: EbookCubit.get(context).getCategoryBooks(category: "Science"),
                builder: (BuildContext context,AsyncSnapshot snapshot) {
                  if (snapshot.hasData) {
                    cubit.scienceSnapshot = snapshot;
              ...
              childAspectRatio: 1/1.7,
              children:
              List.generate(cubit.scienceSnapshot.data['data'].length, (index) => buildGridBook(
                onTap: (){
                  EbookCubit.get(context).index = index;
                  EbookCubit.get(context).isSaved = false;
                  EbookCubit.get(context).books = cubit.scienceSnapshot.data['data'][index];
                  EbookCubit.get(context).bookModel = BookModel.fromJson(
                      cubit.scienceSnapshot.data['data'][index]);
                  navigateTo(context, PreviewBookScreen());
                },
                bookModel: BookModel.fromJson(
                    cubit.scienceSnapshot.data['data'][index]),
                context: context,
                snapshot: cubit.scienceSnapshot,
              ),
              ),
            ),
            ),
